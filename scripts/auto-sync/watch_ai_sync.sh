...
exec "$WATCHEXEC" \
  ...
  -- "version_check=\$(${VENV_PYTHON} \"${ROOT}/scripts/shared/check_client_versions.py\" --check \"${ROOT}/scripts/.client-versions.json\" 2>&1); vc_status=\$?; if [ \"\$vc_status\" -ne 0 ]; then echo \"\$version_check\" >&2; \"${NOTIFY_SCRIPT}\" \"Sync blocked: \$version_check\"; exit \"\$vc_status\"; fi; output=\$(${VENV_PYTHON} \"${SYNC_SCRIPT}\" 2>&1); exit_code=\$?; echo \"\$output\"; summary=\$(${VENV_PYTHON} \"${ROOT}/scripts/shared/sync_summary.py\" 2>/dev/null || true); [ -n \"\$summary\" ] || summary='agents=? skills=? servers=?'; if [ \"\$exit_code\" -eq 0 ]; then msg=\"Sync finished (\$summary)\"; else msg=\"Sync failed (\$summary)\"; fi; \"${NOTIFY_SCRIPT}\" \"\$msg\""
