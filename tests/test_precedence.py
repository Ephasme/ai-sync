...
def test_parse_override_string() -> None:
    assert parse_override("/servers/a/timeout_seconds=5000", parse_json=False) == ("/servers/a/timeout_seconds", "5000")
...
def test_apply_overrides_leaf_only() -> None:
    doc = {"servers": {"a": {"enabled": False, "timeout_seconds": 10}}}
    out = apply_overrides(doc, [("/servers/a/enabled", True), ("/servers/a/timeout_seconds", 20)])
    assert out["servers"]["a"]["enabled"] is True
    assert out["servers"]["a"]["timeout_seconds"] == 20
